...
class CMdtunerSelector:
    ...
    def run_search(self):
        result = self.tuneobj.run_search()

        # Attempt to finalize the best trial for training/export
        try:
            best_trial = self.tuneobj.oracle.get_best_trial()
            ...
            logger.exception("❌ Failed to fetch best trial from OracleClient.")
            return None
