   ...
   class CDataProcess:
      ...
      def _set_ml_features(self, kwargs):
         ...
         self.mlfeatures = self.ml_params.get('mp_features', {})
         
         # Set default Feature parameters
         for feature_key in self.mlfeatures:
               self.mlfeatures[feature_key] = self.ml_params.get(feature_key, "KeyFeature")
         self.feature1 = self.mlfeatures.get("Feature1", "KeyFeature")
         logger.info("Feature1: %s", self.feature1)

         for scaled_feature_key in self.mlfeatures:
               self.mlfeatures[scaled_feature_key] = self.ml_params.get(scaled_feature_key, "KeyFeature_Scaled")
         self.feature1_scaled = self.mlfeatures.get("Feature1_scaled", "KeyFeature_Scaled")
         logger.info("Feature1_scaled: %s", self.feature1_scaled)

         for label_key in self.mlfeatures:
               self.mlfeatures[label_key] = self.ml_params.get(label_key, "Label")
         self.label = self.mlfeatures.get("Label1", "Label")
         ...
         logger.info(f"Machine learning features: {self.mlfeatures}")
         logger.info(f"Machine learning input key feature: {self.mp_ml_input_keyfeat}")
         logger.info(f"Machine learning input key feature scaled: {self.mp_ml_input_keyfeat_scaled}")
         logger.info(f"Machine learning input label: {self.mp_ml_input_label}")
         ...
         logger.info(f"Row number: {self.rownumber}")
         ...
         logger.info(f"Lookahead periods: {self.lookahead_periods}")
         logger.info(f"Moving average window: {self.ma_window}")
         logger.info(f"High-low average column: {self.hl_avg_col}")
         logger.info(f"Moving average column: {self.ma_col}")
         logger.info(f"Returns column: {self.returns_col}")
         logger.info(f"Shift in: {self.shift_in}")
         logger.info(f"Run average: {self.run_avg}")
         logger.info(f"Run average scaled: {self.run_avg_scaled}")
         logger.info(f"Log stationary: {self.log_stationary}")
         logger.info(f"Remove zeros: {self.remove_zeros}")
         logger.info(f"Last column: {self.last_col}")
         logger.info(f"Last column scaled: {self.last_col_scaled}")
         logger.info(f"First column: {self.first_col}")
      ...
      def move_feat_col_to_end(self, df: pd.DataFrame, df_name: str = None) -> pd.DataFrame:
         ...
         if mp_filesrc in self.last_columns and self.last_col:
               ...
               # Assuming feat_col is a tuple; iterate if necessary.
               for col in feat_col:
      ...
      def run_dataprocess_services(self, **kwargs):
         # Process the data wrangling service 
         self.df = kwargs.get('df', pd.DataFrame())
         self.df_name = kwargs.get('df_name', None)
         self.ldf = None
   ...
   # Example main function call (adjust as needed for your application)
   def main(logger):
