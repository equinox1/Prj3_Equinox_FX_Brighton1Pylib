...
"""
#--------------------------------------------------------------------
# create method  "dl_build_neuro_ 1DCNNLSTM ".
# class: cmqlmlsetup
# usage: mql data
# \pdl_build_neuro_network
#--------------------------------------------------------------------
"""
To create a 1D CNN-LSTM hybrid model using TensorFlow and Keras, we combine a Convolutional Neural Network (CNN) 
and a Long Short-Term Memory (LSTM) network. The CNN can handle spatial relationships, while the LSTM can capture 
temporal dependencies, making this hybrid architecture particularly useful for sequence data (e.g., time series, signal processing, etc.).

Key Components:
Conv1D: A 1D convolutional layer to process sequence data.
MaxPooling1D: A pooling layer to downsample the feature maps.
LSTM: A recurrent layer to capture temporal relationships in the sequence data.
Dense: A fully connected layer that outputs the final predictions.
Dropout: Regularization to prevent overfitting.
Flatten: To reshape the data before feeding it into the LSTM layer.

Parameters:
input_shape=(100, 1): Assumes the input is a sequence of length 100 with one feature per time step. Y

Conv1D filters=64, kernel_size=3: These are typical choices for 1D CNNs, but you can experiment with these hyperparameters.
LSTM(100): 100 units in the LSTM cell. You can increase or decrease the number based on your needs.

Customization:
You can modify the number of CNN and LSTM layers, adjust the number of units, change activation functions, or add additional layers (like Batch Normalization).
This architecture is useful for tasks like time series forecasting, signal processing, or any sequence-based classification problem.
"""
    def dl_build_neuro_1DCNNLSTM (self,lp_seq,lp_filt,lp_pool,lp_ksizelp_k_reg, X_train, y_train,optimizer='adam',lp_act1 = 'relu', lp_act2 = 'linear',lp_act3 = 'sigmoid',lp_metric = 'accuracy', lp_loss1='mean_squared_error',lp_loss2 = 'binary_crossentropy',cells1=128,cells2=256, cells3=128,cells4=64,cells5=1):

        # Define the model
        model = Sequential()

        # Add a 1D CNN layer
        model.add(Conv1D(filters=lp_filt, kernel_size=lp_ksize, activation=lp_act1, input_shape=(X_train.shape[1], 1)))
        model.add(MaxPooling1D(pool_size=lp_pool))

        # Add another CNN layer
        model.add(Conv1D(filters=lp_filt, kernel_size=lp_ksize, activation=lp_act1))
        model.add(MaxPooling1D(pool_size=lp_pool))

        # Flatten the output from CNN for LSTM input
        model.add(Flatten())

        # Add LSTM layer
        model.add(LSTM(cells1, return_sequences=lp_seq))

        # Add another LSTM layer
        model.add(LSTM(cells2))

        # Add a Dense layer
        model.add(Dense(cells2, activation=lp_act1))

        # Optional: Add a dropout layer to prevent overfitting
        model.add(Dropout(0.5))

        # Output layer
        model.add(Dense(1, activation=lp_act3))  # For binary classification

        # Compile the model
        model.compile(optimizer=optimizer, loss=lp_loss2, metrics=[lp_metric])
