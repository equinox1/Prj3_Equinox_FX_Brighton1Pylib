...
def main():
    with strategy.scope():
        ...
        params = {name: env.get_params() for name, env in gen_environments.items()}
        tuner_params = {name: env.get_params() for name, env in tuner_environments.items()}
        model_params = {name: env.get_params() for name, env in model_environments.items()}
        
        obj0_CDataLoader = CDataLoader(
            gen_environments["dataenv"], 
            gen_environments["mlenv"], 
            gen_environments["globalenv"],
            tuner_environments["tuneenv"], 
            model_environments["modelenv"]
         )

        obj0_CDataProcess= CDataProcess(
            gen_environments["dataenv"], 
            gen_environments["mlenv"], 
            gen_environments["globalenv"],
            tuner_environments["tuneenv"], 
        ...
        #paramtable=obj0_CDataLoader.display_params(tuner_params,rows,colwidth_env, colwidth_param, colwidth_value, tablefmt="pretty", floatfmt=".5f", numalign="left", stralign="left")
        #print("Tuner Parameters:", tuner_params)
        #print(paramtable)
        ...
        mp_data_history_size = gen_environments["dataenv"].mp_data_history_size if hasattr(gen_environments["dataenv"], 'mp_data_history_size') else 0
        ...
        dataenv = gen_environments["dataenv"]
        mlenv = gen_environments["mlenv"]
        globalenv = gen_environments["globalenv"]
        
        mv_data_utc_from = obj0_CDataLoader.set_mql_timezone(CURRENTYEAR - mp_data_history_size, CURRENTMONTH, CURRENTDAYS, TIMEZONE)
        mv_data_utc_to = obj0_CDataLoader.set_mql_timezone(CURRENTYEAR, CURRENTMONTH, CURRENTDAYS, TIMEZONE)
        ...
        try:
            ...
            obj1_params = CDataLoader(
                dataenv, mlenv, globalenv,tuneenv,modelenv,
                api_ticks=gen_environments["dataenv"].mp_data_loadapiticks,
                api_rates=gen_environments["dataenv"].mp_data_loadapirates,
                file_ticks=gen_environments["dataenv"].mp_data_loadfileticks,
                file_rates=gen_environments["dataenv"].mp_data_loadfilerates,
                dfname1=gen_environments["dataenv"].mv_data_dfname1,
                dfname2=gen_environments["dataenv"].mv_data_dfname2,
                utc_from=mv_data_utc_from,
                symbol_primary=mp_symbol_primary,
                rows=gen_environments["dataenv"].mp_data_rows,
                rowcount=gen_environments["dataenv"].mp_data_rowcount,
                command_ticks=gen_environments["dataenv"].mp_data_command_ticks,
                command_rates=gen_environments["dataenv"].mp_data_command_rates,
            ...
            try:
                mv_tdata1apiticks, mv_tdata1apirates, mv_tdata1loadticks, mv_tdata1loadrates = obj1_params.load_market_data(obj0_CDataProcess, obj1_params)
        ...
        if  not loadmql:
            mv_tdata1apiticks = obj0_CDataProcess.wrangle_time(mv_tdata1apiticks, mp_unit, mp_filesrc="ticks1", filter_int=False, filter_flt=False, filter_obj=False, filter_dtmi=False, filter_dtmf=False, mp_dropna=False, mp_merge=False, mp_convert=False, mp_drop=True)
        if not loadmql:
            mv_tdata1apirates = obj0_CDataProcess.create_label_wrapper(
                ...
                column_in="R1_Bid_Price",  # Corrected parameter
                column_out1='Close',
            ...
            mv_tdata1loadticks = obj0_CDataProcess.wrangle_time(mv_tdata1loadticks, mp_unit, mp_filesrc="ticks2", filter_int=False, filter_flt=False, filter_obj=False, filter_dtmi=False, filter_dtmf=False, mp_dropna=False, mp_merge=True, mp_convert=True, mp_drop=True)
            mv_tdata1loadrates = obj0_CDataProcess.wrangle_time(mv_tdata1loadrates, mp_unit, mp_filesrc="rates2", filter_int=False, filter_flt=False, filter_obj=False, filter_dtmi=False, filter_dtmf=False, mp_dropna=False, mp_merge=True, mp_convert=True, mp_drop=True)

        scolumn_out1 = gen_environments["dataenv"].mp_data_custom_input_keyfeat
        scolumn_out2 = gen_environments["dataenv"].mp_data_custom_output_label_scaled
