#!/usr/bin/env python3
# +------------------------------------------------------------------+
# | super_launcher_autorestart.py (Launch + Monitor + Auto-Restart)   |
# +------------------------------------------------------------------+
...
def monitor_process(name, script, env_vars, tf_config, restart=True):
    while True:
        proc = launch_process(script, env_vars, name, tf_config)
        print(f"✅ {name} launched with PID {proc.pid}")

        # Wait for process to finish
        proc.wait()
        print(f"❗ {name} exited with return code {proc.returncode}")
...
def main():
    ...
    chief_thread = threading.Thread(target=monitor_process, args=("chief", chief_script, chief_env, chief_tf_config))
    chief_thread.start()

    time.sleep(2)  # Small wait for Chief to stabilize
    ...
    for i in range(num_workers):
        ...
        thread = threading.Thread(target=monitor_process, args=(tuner_id, worker_script, worker_env, worker_tf_config))
        thread.start()
    ...
    print("\n⏳ System launched. Watching logs. Auto-restarting on crash. Press Ctrl+C to exit monitor.\n")
